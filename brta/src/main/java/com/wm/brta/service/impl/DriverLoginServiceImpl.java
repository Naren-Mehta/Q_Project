...
@Component(value = "driverLoginService")
@Transactional
	...
	public UserDTO authenticate(User inputUser) throws Exception {
		...
		if (users != null && users.size() > 0) {
			...
			if(users.size() == 1){
				...
				List<Role> roles = userDao.getAllRoleFromUser(user, "Driver"); // role
				...
				if (roalMatched) {
					...
					userDTO.setMessage(ApplicationCommonConstants.DRIVER_AUTHENTICATION_SUCCESSFULL_MESSAGE);
				} else {
					...
					userDTO.setMessage(ApplicationCommonConstants.ROLE_AUTHENTICATION_UNSUCCESSFULL_MESSAGE);
				}
			}else{
				userDTO.setFirstName(null);
				userDTO.setLastName(null);
			...
			userDTO.setMobilePhone(inputUser.getMobilePhone());
			userDTO.setAuthenticationErrorFlag(true);
			userDTO.setMessage(ApplicationCommonConstants.DRIVER_AUTHENTICATION_UNSUCCESSFULL_MESSAGE);
			return userDTO;
